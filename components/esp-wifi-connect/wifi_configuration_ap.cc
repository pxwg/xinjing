...
void WifiConfigurationAp::StartWebServer() {
  ...
  httpd_uri_t form_submit = {
      ...
      .handler = [](httpd_req_t *req) -> esp_err_t {
        ...
        cJSON *username_item = cJSON_GetObjectItemCaseSensitive(
            json, "username"); // Parse username
        ...
        if (cJSON_IsString(username_item) &&
            ...
            (strlen(username_item->valuestring) < 65)) {
...
bool WifiConfigurationAp::ConnectToWifi(const std::string &ssid,
  ...
  // 1. 配置认证模式
  if (!username.empty()) {
    ESP_LOGI(TAG, "Test connecting WPA2-Ent: %s (User: %s)", ssid.c_str(),
             username.c_str());
    ESP_ERROR_CHECK(esp_eap_client_set_identity(
        (const uint8_t *)username.c_str(), username.length()));
    ...
    ESP_LOGI(TAG, "Test connecting Personal: %s", ssid.c_str());
  ...
  // 4. 等待连接结果（超时时间设为 15秒，给予 Enterprise 足够握手时间）
  EventBits_t bits =
  ...
  if (bits & WIFI_CONNECTED_BIT) {
    ...
    ESP_LOGW(TAG, "❌ Test Connection Failed (Timeout or Auth Error)");
    return false;
  }
}
...
void WifiConfigurationAp::Save(const std::string &ssid,
                               const std::string &password,
                               const std::string &username) {
  ESP_LOGI(TAG, "Save SSID %s %d (User: %s)", ssid.c_str(), ssid.length(),
           username.c_str());
